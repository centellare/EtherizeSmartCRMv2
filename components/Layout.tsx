...
const Layout: React.FC<LayoutProps> = ({ children, profile, activeModule, setActiveModule, onProfileUpdate }) => {
  ...
  // Фильтрация меню согласно требованиям безопасности
  const menuItems = allMenuItems.filter(item => {
    const role = profile?.role;
    if (role === 'specialist') {
      return ['dashboard', 'tasks', 'objects'].includes(item.id);
    }
    if (role === 'manager') {
      return !['database', 'trash'].includes(item.id);
    }
    if (role === 'director') {
      return item.id !== 'database';
    }
    return true; // Admin видит всё
  });
        ...
        <form onSubmit={handleProfileUpdate} className="space-y-6">
          <div className="flex flex-col items-center mb-6">
            ...
            <div className="flex gap-2 mt-1">
              <Badge color="blue">{profile?.role?.toUpperCase() || 'USER'}</Badge>
            </div>
            {profile?.created_at && (
              <p className="text-[10px] text-slate-400 uppercase font-bold mt-4 tracking-widest">
                В системе с: {new Date(profile.created_at).toLocaleDateString()}
              </p>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Input 
                ...
                onChange={(e:any) => setProfileForm({...profileForm, birth_date: e.target.value})} 
                icon="calendar_today" 
              />
            </div>
          </div>
        ...
        <nav className="flex-grow px-4 space-y-1 overflow-y-auto scrollbar-hide">
          {menuItems.map((item) => (
            <button 
              ...
              className={`w-full flex items-center gap-4 px-4 h-12 rounded-full text-sm font-medium transition-all group ${
                activeModule === item.id 
                  ? 'bg-[#d3e4ff] text-[#001d3d]' 
                  : 'text-[#444746] hover:bg-black/5'
              }`}
            >
              <span className={`material-icons-round ${activeModule === item.id ? 'text-[#001d3d]' : 'text-[#444746] group-hover:text-[#1c1b1f]'}`}>
                {item.icon}
              </span>
              {item.label}
            </button>
          ...
          <div className="bg-white rounded-2xl p-4 border border-[#e1e2e1] shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <div 
                ...
                className="w-10 h-10 rounded-full bg-[#eff1f8] border border-[#d3e4ff] flex items-center justify-center text-[#005ac1] font-bold text-xs cursor-pointer hover:bg-[#d3e4ff] transition-colors"
              >
                {userInitials}
              </div>
              <div className="flex-grow min-w-0">
                <p className="text-sm font-bold text-[#1c1b1f] truncate leading-tight">
                  {profile?.full_name || 'Загрузка...'}
                </p>
                <div className="mt-1">
                  <Badge color="slate">{profile?.role || 'user'}</Badge>
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              <button 
                onClick={() => setIsProfileModalOpen(true)}
                className="flex-1 h-9 flex items-center justify-center gap-2 rounded-xl border border-[#e1e2e1] text-[#444746] hover:bg-[#f3f5f7] transition-all text-xs font-medium"
              >
              ...
              <button 
                ...
                className="w-9 h-9 flex items-center justify-center rounded-xl border border-[#ffdad6] text-[#ba1a1a] hover:bg-[#ffdad6] transition-all"
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-[#e1e2e1] flex items-center justify-between px-8 shrink-0">
          <div className="flex items-center gap-4">
            <span className="material-icons-round text-[#444746]">
              {allMenuItems.find(i => i.id === activeModule)?.icon}
            </span>
            <h3 className="text-lg font-medium text-[#1c1b1f]">
              {allMenuItems.find(i => i.id === activeModule)?.label}
            </h3>
          </div>
          <div className="flex items-center gap-4">
            <button 
              onClick={() => setActiveModule('notifications')}
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors relative ${activeModule === 'notifications' ? 'bg-[#d3e4ff] text-[#001d3d]' : 'hover:bg-[#f3f5f7] text-[#444746]'}`}
            >
