...
export const FinancesTab: React.FC<FinancesTabProps> = ({
  ...
  const objectSummary = useMemo(() => {
    // Для карточек используем транзакции, отфильтрованные ТОЛЬКО по дате
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
       <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="flex gap-2 items-center">
             ...
             <Button variant="tonal" icon="request_quote" onClick={() => { resetForm(); setFormData(p => ({...p, type: 'expense'})); setIsModalOpen(true); }} className="text-xs h-10">Внести расход</Button>
             {!isSpecialist && <Button icon="add_chart" onClick={() => { resetForm(); setFormData(p => ({...p, type: 'income'})); setIsModalOpen(true); }} className="text-xs h-10">План прихода</Button>}
          </div>
       </div>
