            ...
            >
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="flex gap-2">
                  ...
                  {linkedInvoice && (
                    <div 
                      onClick={(e) => { e.stopPropagation(); if (onViewInvoice) onViewInvoice(linkedInvoice.id); }}
                      className="flex items-center gap-1 text-[10px] font-bold text-white bg-emerald-500 px-2 py-1 rounded cursor-pointer hover:bg-emerald-600 transition-colors shadow-sm"
                      title="Открыть счет"
                    >
                      <span className="material-icons-round text-[10px]">receipt</span>
                      Счет №{linkedInvoice.number}
                    </div>
                  )}
                </div>
              </div>
