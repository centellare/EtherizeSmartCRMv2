...
const Finances: React.FC<{ profile: any }> = ({ profile }) => {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      {!isSpecialist && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
           <div onClick={() => setSummaryFilter(summaryFilter === 'cash' ? null : 'cash')} className={`bg-white p-6 rounded-3xl border transition-all cursor-pointer hover:shadow-lg ${summaryFilter === 'cash' ? 'border-emerald-500 ring-4 ring-emerald-500/10' : 'border-[#e1e2e1] shadow-sm'}`}>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Касса (Баланс)</p>
              <p className={`text-2xl font-bold ${totals.balance >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{formatBYN(totals.balance)}</p>
           </div>
           ...
           <div onClick={() => setSummaryFilter(summaryFilter === 'total_income' ? null : 'total_income')} className={`bg-white p-6 rounded-3xl border transition-all cursor-pointer hover:shadow-lg ${summaryFilter === 'total_income' ? 'border-slate-800 ring-4 ring-slate-800/10' : 'border-[#e1e2e1] shadow-sm'}`}>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Всего приходов (Факт)</p>
        ...
        {selectedTrans && (
          <form onSubmit={handleAddPayment} className="space-y-4">
            {(() => {
              ...
              return (
                <>
                  <div className={`p-4 rounded-2xl transition-colors ${isOverpaid ? 'bg-red-50' : 'bg-emerald-50'}`}>
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setPaymentAmount(balanceLeft.toFixed(2))} className="absolute right-3 top-9 px-3 py-1 bg-emerald-600 text-white text-[10px] font-bold uppercase rounded-lg hover:bg-emerald-700">Вся сумма</button>
                  </div>
                  <Input label="Комментарий" value={paymentComment} onChange={(e:any) => setPaymentComment(e.target.value)} icon="comment" />
                  <Button type="submit" className="w-full h-12" loading={loading} icon="check" disabled={isOverpaid || currentVal <= 0}>Подтвердить</Button>
                </>
              );
            })()}
          </form>
      ...
      <ConfirmModal isOpen={isDeleteConfirmOpen} onClose={() => setIsDeleteConfirmOpen(false)} onConfirm={handleSoftDelete} title="Удалить операцию?" message="Транзакция будет перемещена в корзину." confirmVariant="danger" loading={loading} />
      <ConfirmModal isOpen={isFinalizeConfirmOpen} onClose={() => setIsFinalizeConfirmOpen(false)} onConfirm={handleFinalizeIncome} title="Завершить приход?" message="План будет приравнен к фактической сумме. Дебиторская задолженность по этой записи будет обнулена." confirmVariant="tonal" loading={loading} />
    </div>
  );
};
